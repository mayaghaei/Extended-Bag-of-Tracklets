...
im = imread('G:\pedestrian_tracking\NarrativeDB\group\9\172855.jpg');
...
%     try
    ...
    x(i) = min(bs(i).xy(:,1));
    y(i) = min(bs(i).xy(:,2));
    w(i) = max(bs(i).xy(:,3)) - min(bs(i).xy(:,1));
    h(i) = max(bs(i).xy(:,4)) - min(bs(i).xy(:,2));
    end
    imshow(im)
    hold on
    for j = 1:length(bs)
    rectangle('position', [x(j) y(j) w(j) h(j)]);
    hold off
    pause;
    end
